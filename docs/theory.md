...
## 4. Магія векторизації (SIMD) та чому i-k-j працює ще швидше
...
## 5. Вартість абстракцій та структура даних (2D vs 1D масиви)

Ми успішно розв'язали проблему "стрибків" (Cache Misses), обравши правильний алгоритм `i-k-j`. Здавалося б, тепер пам'ять читається лінійно (і SIMD працює). Але є ще одна архітектурна проблема: абстракція, яку ми вибрали для зберігання матриці.

У класичному ООП ми звикли описувати двовимірну матрицю як масив масивів (`vector<vector<int>>`). Це інтуїтивно зрозуміло для людини, але створює зайвий клопіт для процесора:
- **Ціна подвійних вказівників:** Масив масивів — це насправді масив вказівників на інші масиви. Щоб знайти елемент `B[k][j]`, програма спершу йде в пам'ять за адресою рядка `k`, а вже потім — за його елементом `j`.
- **Фрагментація пам'яті:** Рядки матриці виділяються (алокуються) незалежно один від одного і фізично розташовуються розпорошено по оперативній пам'яті. Це може обривати ідеальну роботу кешу і вимикати SIMD-магію на стиках рядків.

**Рішення: Плоский одновимірний масив (Flat 1D Vector)**
Професіонали у сфері High Performance Computing (HPC) "сплющують" матрицю в один величезний суцільний одновимірний шматок пам'яті: `vector<int> flat(n*n)`. Це гарантує, що вся матриця лежить ідеально лінійно і неперервно від першого до останнього байта. Доступ до потрібного рядка та стовпця `[i][j]` перетворюється на швидку математику безпосередньо в регістрі процесора: `flat[i * n + j]`.

Ця специфічна 1D структура даних критично важлива для розкриття повного потенціалу векторизації (SIMD-магії) без обривів. Більше того, цей самий підхід є фундаментальним для роботи з відеокартами (GPU Memory Coalescing).

---

## 6. Порівняльна таблиця методів

Давайте підіб'ємо підсумки розвитку нашого коду від базової математики до серйозної інженерії:

| Характеристика | Базовий: `i-j-k` (2D) | Оптимізований: `i-k-j` (2D) | Ультимативний: `i-k-j` (Flat 1D) |
| --- | --- | --- | --- |
| **Внутрішній цикл проходить по** | `k` (стовпцю) | `j` (рядку) | `j` (рядку) |
| **Доступ до пам'яті (Cache Hits)**| Низька (багато дорогих промахів) | Максимально висока (~4%) | Максимально висока (~4%) |
| **Структура в оперативній пам'яті** | Масив вказівників (Fragments) | Масив вказівників (Fragments) | Суцільний блок пам'яті (Continuous) |
| **SIMD-векторизація** | Майже неможлива ❌ | Добра ✅ | Ідеальна (без обривів) 🚀 |
| **Очікувана швидкість C++** | Повільно | Рівно в 18 разів швидше | Найшвидше (мінімум інструкцій) |

Як бачимо, ми зібрали всі оптимізації воєдино: ідеальний для кешу математичний порядок та ідеальну для процесора (SIMD) "плоску" структуру пам'яті!

---

## 7. C++ Code Snippets (Приклади реалізації)

Ось процедурна реалізація на C++ для всіх трьох підходів. Зверніть увагу, як змінюється структура доступу до даних в останньому алгоритмі.
...
### Ультимативний "Найшвидший" Алгоритм (Плоский 1D Масив)
```cpp
// Забороняємо компілятору зливати (inline) цю функцію, щоб вона відображалась на Flame Graph
[[gnu::noinline]]
void multiplyMatrixFlat(const vector<int>& A, const vector<int>& B, vector<int>& C, int n) {
    for (int i = 0; i < n; i++) {
        for (int k = 0; k < n; k++) {
            int a_ik = A[i * n + k];
            for (int j = 0; j < n; j++) {
                // Математика та сама, але масив тепер суцільний (одновимірний).
                // Усі рядки матриці лежать один за одним без розривів.
...
## 8. Практика: Дивимося на кеш-промахи своїми очима (Flame Graphs)

Ми вже знаємо теорію про "широкі штампи", SIMD та читання "підряд по рядках", але як подивитися всередину справжнього процесора під час роботи алгоритму? 
...
4. **Чому графік такий "плоский" (або куди зникли всі інші функції)?** На відміну від звичайних програм з ООП, тут ми маємо справу з High Performance Computing алгоритмом, скомпільованим з прапорцем максимальної оптимізації **`-O3`**. Цей прапорець агресивно "розчинив" (inlined) усі виклики методів (наприклад, `size()` чи звернення до вектора `[]`) у чисті машинні інструкції прямо всередині нашого циклу. Оскільки в тілі не залишилося викликів "справжніх" функцій, `perf` малює один монолітний блок. Це ідеальний доказ "чистоти" експерименту — графік не засмічений сторонніми процесами, і ми бачимо виключно вплив алгоритму на пам'ять!
5. **Секрет 4.1%: Чому 2D (`IKJ`) та 1D (`Flat`) майже однакові на графіку?** Обидва варіанти майже ідеально розв'язують проблему кешу, бо читають дані зліва направо (промахи падають з 90% до ~4.1%). Але тут ховається геніальність компілятора C++. Прапорець `-O3` розуміє, що рядок `B[k]` не змінюється у найвнутрішнішому циклі `j` (який крутиться 8 мільярдів разів). Тому компілятор автоматично "підіймає" пошук цього вказівника на рівень вище. Як показують ручні перевірки `perf stat`, плоский 1D-масив дозволяє зекономити ще близько 18 мільйонів інструкцій процесора (завдяки відсутності подвійних стрибків за вказівниками рядків навіть перед циклами). Профілювальник підтверджує: правильний алгоритм роботи з пам'яттю + сучасний оптимізатор здатні витиснути максимум із заліза незалежно від надбудованих абстракцій!

### Альтернативний вигляд: Вкладка "Sandwich"
Якщо ви перемкнетесь на вкладку **Sandwich** (вгорі по центру), ви побачите табличний аналіз усіх функцій. Цей вигляд дозволяє кількісно оцінити навантаження та відповісти на запитання "де саме болить найбільше":
- **Total (Загалом):** Скільки всього кеш-промахів згенерувала функція разом з усіма іншими функціями, які вона викликала. Наприклад, `multiplyMatrixIJK` матиме показник близько `95%`, тобто вона відповідальна за 95% усіх проблем із пам'яттю у програмі. А функція `main` матиме понад `99%`, бо вона запустила весь цей процес.
- **Self (Власне):** Скільки кеш-промахів згенеровано *безпосередньо* всередині функції (ігноруючи її виклики інших функцій). Для `multiplyMatrixIJK` він становитиме близько `90%`, а для викликів ядра `???` — близько `5%` (що в сумі якраз дає `Total` = `95%`). А от `Self` для `main` дорівнюватиме `0%`, оскільки вона лише віддавала накази й не зачіпала масиви сама.
- На самому дні списку ви можете помітити дрібні системні функції на кшталт `malloc`, `operator new`, `__printf_fp` (виділення пам'яті та друк у консоль). Їхня вага ледь досягає `<0.01%`. Це ще один доказ того, що ініціалізація навіть таких гігантських матриць 2048x2048 практично нічого не важить для процесора порівняно з часом, який втрачається через помилки в обході пам'яті!
