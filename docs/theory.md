...
**Чому `vector<vector<int>>` — це погана абстракція для розрахунків?**
Коли ви створюєте `vector<vector<int>>`, ви створюєте не одну велику матрицю, а масив вказівників, кожен з яких вказує на окремий масив-рядок. Ці масиви-рядки виділяються (алокуються) окремо і можуть бути розкидані по різних кутках оперативної пам'яті. Кожен раз при переході на новий рядок процесор обриває послідовність читання і стрибає на нову адресу, збиваючи кеш і зупиняючи швидкі інструкції (SIMD).
Натомість суцільний одновимірний масив (`vector<int> flat(n*n)`) "випрямляє" пам'ять. Це гарантує, що вся гіганська матриця лежить ідеально лінійно та підряд (Continuous Block) від першого до останнього байта. Процесори вимагають саме такого ідеально рівного доступу для масових паралельних операцій.
...
- **Секрет 4.1%: Чому 2D (`IKJ`) та 1D (`Flat`) майже однакові за кеш-промахами?** Обидва варіанти з правильним циклом `i-k-j` ідеально розв'язують проблему кешу, бо читають дані зліва направо (промахи кешу падають з 90% до ~4.1%). Але тут ховається геніальність компілятора C++. Прапорець `-O3` достатньо розумний, щоб зрозуміти, що рядок `B[k]` не змінюється у найвнутрішнішому циклі `j` (який крутиться 8 мільярдів разів). Тому він автоматично "підіймає" пошук цього вказівника на рівень вище. Як показують додаткові перевірки `perf stat`, плоский 1D-масив дозволяє зекономити ще близько 18 мільйонів інструкцій процесора за рахунок повної відсутності навіть рідкісних стрибків за вказівниками рядків. Профілювальник підтверджує: правильний алгоритм + сучасний компілятор здатні витиснути максимум із пам'яті незалежно від абстракції.
