...
- **Секрет 4.1%: Чому 2D (`IKJ`) та 1D (`Flat`) майже однакові за кеш-промахами, але 1D масив швидший на практиці?** Обидва варіанти з правильним циклом `i-k-j` ідеально розв'язують проблему кешу, бо читають дані зліва направо (відсоток промахів падає з 90% до ~4.1%). Перевага Плоского 1D-масиву лежить не стільки в області кешу, скільки в інструкціях: процесору більше не треба двічі сходити в пам'ять за вказівниками рядків (`A[i] -> [k]`). Замість цього він робить миттєву арифметику в регістрах (`i * n + k`), що скорочує загальну кількість інструкцій на сотні мільйонів і дозволяє алгоритмам векторизації (SIMD) безперешкодно об'єднувати дані. Отже, Flame Graph доводить, що ми вилікували проблему "голодування" пам'яті в обох методах, а Flat Memory додатково зняв з процесора обчислювальний тягар.
