...
Ми інтегрували професійний переглядач `Speedscope` прямо в цю сторінку! Графік повинен завантажуватися всередині вікна автоматично.

<iframe src="./speedscope/index.html#profileURL=../cache-misses.txt" width="100%" height="600" style="border: 2px solid #ccc; border-radius: 8px; margin-bottom: 20px;"></iframe>

> Якщо вікно залишилось синім і графік не відобразився (іноді так буває через налаштування браузера), просто [👉 збережіть файл cache-misses.txt](./cache-misses.txt) на комп'ютер та перетягніть його самостійно всередину Speedscope вище.

### Що ви там побачите (і як це читати)?
Уявіть, що Flame Graph — це розріз вашого коду як листкового пирога. 
- **Зверху вниз (по осі Y):** показується ланцюг того, як функції викликали одна одну. Наприклад, спочатку запустився `main`, який потім викликав `multiplyMatrixIJK`.
- **Ширина блоку (Це найголовніше!):** показує, **скільки кеш-промахів** згенерувала ця конкретна функція! Чим ширший блок — тим частіше процесор спотикався, чекаючи на завантаження даних із повільного "підвалу" оперативної пам'яті.

**Що треба перевірити:**
Перемкніться на вкладку **"Left Heavy"** (вгорі зліва екрана Speedscope). Вона групує всі виклики. Знайдіть і порівняйте два прямокутники:

1. Прямокутник `multiplyMatrixIJK` (наш класичний математичний метод) буде неймовірно **широким і масивним**. Він згенерував левову частку всіх кеш-промахів програми через стрибки по стовпцю вниз.
2. Прямокутник `multiplyMatrixIKJ` (оптимізований метод) буде **дуже вузьким**. Процесорні кеш-промахи там практично відсутні, тому що дані бралися з пам'яті ідеально підряд (по горизонталі).
